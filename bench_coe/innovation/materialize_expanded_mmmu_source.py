...
def _prediction_path(root: Path, expert: str) -> Path:
    ...
    for name in ("predictions.json", "predictions.jsonl"):
        ...
        if path.exists():
            return path
    raise FileNotFoundError(f"Missing raw prediction cache for {expert}: {model_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Project the mixed MMMU-Pro cache onto an ID-defined source split"
    )
    ...
    for expert in experts:
        ...
        if canonical_ids is None:
            ...
            raise RuntimeError(f"{expert} projected source IDs differ from the common ID set")
    ...
    manifest = {
        ...
        "output_source_cache_hashes": output_hashes,
        "contains_source_labels": True,
        "contains_target_rows": False,
    }
    manifest_path = args.output_dir / "source_projection_manifest.json"

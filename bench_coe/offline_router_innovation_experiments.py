...
def lcb_for_signed_values(values: list[float], z: float) -> tuple[float, float]:
    if not values:
        return 0.0, 0.0
    mean = sum(values) / len(values)
    variance = sum((value - mean) ** 2 for value in values) / len(values)
    lower = mean - z * math.sqrt(variance / max(len(values), 1))
    return mean, lower


def robust_minimax_model(
    rows: list[dict[str, Any]],
    probe_ids: list[str],
    matrix: dict[str, dict[str, bool]],
    key: str,
    ...
    for model, values in sorted(matrix.items()):
        group_lcbs: list[float] = []
        correct_total = 0
        for ids in by_group.values():
            correct = sum(1 for rid in ids if values.get(rid, False))
            correct_total += correct
            group_lcbs.append(wilson_lower(correct, len(ids), z))
        score = min(group_lcbs) if group_lcbs else 0.0
        mean_acc = correct_total / len(probe_ids) if probe_ids else 0.0
        model_scores[model] = {"min_group_lcb": score, "probe_accuracy": mean_acc}
        if score > best_score or (score == best_score and mean_acc > best_mean):
            best_model = model
            best_score = score
            best_mean = mean_acc
    return best_model, {
        "robust_key": key,
        "num_probe_groups": len(by_group),
        "min_group_lcb": best_score,
        "probe_accuracy": best_mean,
        "top_models": dict(
            sorted(
                model_scores.items(),
                key=lambda item: (-item[1]["min_group_lcb"], -item[1]["probe_accuracy"], item[0]),
            )[:5]
        ),
    }


def failure_boundary_group_choices(
    rows: list[dict[str, Any]],
    probe_ids: list[str],
    eval_ids: list[str],
    matrix: dict[str, dict[str, bool]],
    ...
    row_by_id = {row_id(row): row for row in rows}
    current = current_choice_map(rows, matrix, fallback_model)
    by_group: dict[str, list[str]] = defaultdict(list)
    for rid in probe_ids:
        by_group[group_value(row_by_id[rid], key)].append(rid)
    ...
    for group, ids in sorted(by_group.items()):
        deltas: list[float] = []
        for rid in ids:
            current_model = current[rid]
            current_ok = matrix[current_model].get(rid, False)
            guard_ok = matrix[guard_model].get(rid, False)
            if guard_ok and not current_ok:
                deltas.append(1.0)
            elif current_ok and not guard_ok:
                deltas.append(-harm_weight)
            else:
                deltas.append(0.0)
        mean_delta, lower_delta = lcb_for_signed_values(deltas, z)
        group_scores[group] = {
            "mean_delta": mean_delta,
            "lower_delta": lower_delta,
            "samples": float(len(ids)),
        }
        if lower_delta > margin:
            switch_groups.add(group)

    choices: dict[str, str] = {}
    switches = 0
    for rid in eval_ids:
        group = group_value(row_by_id[rid], key)
        if group in switch_groups:
            choices[rid] = guard_model
            switches += 1
        else:
            choices[rid] = current[rid]
    ...
    return choices, {
        ...
        "harm_weight": harm_weight,
        "margin": margin,
        "switched_groups": len(switch_groups),
        "num_probe_groups": len(by_group),
        "switch_rate": switches / len(eval_ids) if eval_ids else 0.0,
        "top_boundary_groups": dict(
            sorted(group_scores.items(), key=lambda item: -item[1]["lower_delta"])[:8]
        ),
    }


def dynamic_subject_tfidf_choices(
    rows: list[dict[str, Any]],
    probe_ids: list[str],
    eval_ids: list[str],
    matrix: dict[str, dict[str, bool]],
    ...
    if len(probe_ids) < 5 or len(eval_ids) < 10:
        return {rid: global_model for rid in eval_ids}, {"skipped": "not enough probe/eval rows"}
    try:
    ...
    row_by_id = {row_id(row): row for row in rows}
    n_clusters = min(max(2, clusters), max(2, len(all_ids) // 2))
    vectorizer = TfidfVectorizer(
        analyzer="word",
        ngram_range=(1, 2),
        min_df=1,
    ...
    features = vectorizer.fit_transform([text_for_row(row_by_id[rid]) for rid in all_ids])
    kmeans = MiniBatchKMeans(
        n_clusters=n_clusters,
        random_state=seed,
        batch_size=min(2048, max(256, len(all_ids))),
        n_init=5,
    )
    labels = kmeans.fit_predict(features)
    cluster_by_id = {rid: int(label) for rid, label in zip(all_ids, labels)}

    probe_by_cluster: dict[int, list[str]] = defaultdict(list)
    for rid in probe_ids:
        probe_by_cluster[cluster_by_id[rid]].append(rid)
    cluster_to_model: dict[int, str] = {}
    for cluster_id, ids in probe_by_cluster.items():
        if ids:
            model, _, _ = best_model_with_wilson(matrix, ids, z)
            cluster_to_model[cluster_id] = model
    choices = {
        rid: cluster_to_model.get(cluster_by_id[rid], global_model)
        for rid in eval_ids
    }
    used_clusters = {cluster_by_id[rid] for rid in eval_ids}
    return choices, {
        "clusters": n_clusters,
        "clusters_with_probe": len(probe_by_cluster),
        "clusters_in_eval": len(used_clusters),
        "features": len(vectorizer.vocabulary_),
        "global_model": global_model,
    }


def run_dynamic_subject_tfidf_grid(
    rows: list[dict[str, Any]],
    probe_ids: list[str],
    eval_ids: list[str],
    matrix: dict[str, dict[str, bool]],
    ...
    for clusters in cluster_values:
        choices, meta = dynamic_subject_tfidf_choices(
            rows, probe_ids, eval_ids, matrix, global_model, clusters, seed, z
        )
        out.append(
            {
                "method": f"probe_dynamic_subject_tfidf_c{clusters}",
                "accuracy": accuracy_for_choices(eval_ids, choices, matrix),
...
def run_craft_repair_lcb_grid(
    ...
    if len(probe_ids) < 5 or len(eval_ids) < 10:
        return [
            {
                "method": "probe_craft_current_repair_lcb_grid",
                "accuracy": "",
                "meta_skipped": "not enough probe/eval rows",
            }
        ]
    ...
    current_probe_correct = np.array(
        [1.0 if matrix[current[rid]].get(rid, False) else 0.0 for rid in probe_ids]
    )
    model_index = {model: idx for idx, model in enumerate(models)}
    current_eval_indices = np.array([model_index[current[rid]] for rid in eval_ids])
    eval_rows: list[dict[str, Any]] = []
    for harm_weight in harm_weights:
        fixes_current_error = (correctness == 1.0) & (current_probe_correct == 0.0)
        breaks_current_success = (correctness == 0.0) & (current_probe_correct == 1.0)
        deltas = fixes_current_error.astype(float) - harm_weight * breaks_current_success.astype(float)
        priors = deltas.mean(axis=1) if deltas.shape[1] else np.zeros(len(models))
        second_priors = (deltas * deltas).mean(axis=1) if deltas.shape[1] else np.zeros(len(models))
        positive_prior_models = int((priors > 0).sum())
        for neighbors in neighbor_values:
            ...
            for z_value in z_values:
                ...
                for idx, current_idx in enumerate(current_eval_indices):
                    lcb_matrix[current_idx, idx] = min_switch_lcb
                best_indices = np.argmax(lcb_matrix, axis=0)
                best_lcbs = lcb_matrix[best_indices, np.arange(len(eval_ids))]
                choice_indices = np.where(best_lcbs > min_switch_lcb, best_indices, current_eval_indices)
                switches = int(np.sum(choice_indices != current_eval_indices))
                acc = float(eval_correctness[choice_indices, np.arange(len(eval_ids))].mean())
                eval_rows.append(
                    {
                        "method": (
                            f"probe_craft_current_repair_lcb_k{neighbors}"
                            f"_h{harm_weight_tag(harm_weight)}_z{float_tag(z_value)}"
                        ),
                        "accuracy": acc,
                        "meta_neighbors": k,
                        ...
                        "meta_features": len(vectorizer.vocabulary_),
                        "meta_metadata_keys": ",".join(metadata_keys),
                        "meta_baseline": "current_router",
                    }
                )
    return eval_rows


def craft_current_repair_lcb_choices(
    rows: list[dict[str, Any]],
    probe_ids: list[str],
    eval_ids: list[str],
    matrix: dict[str, dict[str, bool]],
    ...
    if len(probe_ids) < 5 or len(eval_ids) < 10:
        current = current_choice_map(rows, matrix, fallback_model)
        return {rid: current[rid] for rid in eval_ids}, {"skipped": "not enough probe/eval rows"}
    try:
        import numpy as np
        from sklearn.feature_extraction.text import TfidfVectorizer
    except Exception as exc:  # pragma: no cover
        current = current_choice_map(rows, matrix, fallback_model)
        return {rid: current[rid] for rid in eval_ids}, {"fallback": f"sklearn/numpy unavailable: {exc}"}
    ...
    for idx, rid in enumerate(eval_ids):
        ...
        lcb[current_idx] = min_switch_lcb
        ...
        if best_lcb > min_switch_lcb:
            ...
            model = current_model
        choices[rid] = model
        switches += 1 if model != current_model else 0
...
def evaluate_probe_budget(
    ...
    for key in group_keys:
        ...
        rows_out.append(
            {
                "method": f"probe_robust_minimax_{key}_global",
                "budget_per_group": budget,
                "split_key": split_key,
                "probe_samples": len(probe_ids),
                "eval_samples": len(eval_ids),
                "accuracy": accuracy_for_model(matrix[robust_model], eval_ids),
                "selected_model": robust_model,
                **{f"meta_{meta_key}": meta_value for meta_key, meta_value in robust_meta.items()},
            }
        )
        boundary_choices, boundary_meta = failure_boundary_group_choices(
            rows=rows,
            probe_ids=probe_ids,
            eval_ids=eval_ids,
            matrix=matrix,
        ...
        rows_out.append(
            {
                "method": f"probe_failure_boundary_{key}_guard",
                "budget_per_group": budget,
                "split_key": split_key,
                "probe_samples": len(probe_ids),
                "eval_samples": len(eval_ids),
                "accuracy": accuracy_for_choices(eval_ids, boundary_choices, matrix),
                "selected_model": robust_model,
                **{f"meta_{meta_key}": meta_value for meta_key, meta_value in boundary_meta.items()},
            }
        )
    ...
    if run_multilabel:
        ...
        rows_out.extend(
            ...
            for row in run_dynamic_subject_tfidf_grid(
                rows,
                probe_ids,
                eval_ids,
                matrix,
                global_model,
                cluster_values=(4, 8, 16, 32),
                seed=seed,
                z=wilson_z,
            )
        )
        rows_out.extend(
            ...
            for row in run_craft_repair_lcb_grid(
                rows,
                probe_ids,
                eval_ids,
                matrix,

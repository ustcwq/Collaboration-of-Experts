...
LOG_ROOT=$OUTPUT_ROOT/logs/front4_ready_models
STATE_ROOT=$OUTPUT_ROOT/state/front4_ready_models
TEXT_MODELS=(
  gemma-2-2b-it
  Ministral-3-3B-Instruct-2512
  Llama-3.2-3B-Instruct
)
VISION_MODEL=GLM-4.1V-9B-Thinking
...
run_task() {
  ...
  local task_log="$LOG_ROOT/$task.log"
  mkdir -p "$(dirname "$completed")" "$(dirname "$task_log")"
...
gpu0_chain() {
  ...
  for model in "${TEXT_MODELS[@]}"; do
    run_task 0 "$model/official_bbh_gpqa_mmstar" \
...
gpu1_chain() {
  local model
  for model in "${TEXT_MODELS[@]}"; do
    run_task 1 "$model/mmlu_pro_test" \
...
gpu2_chain() {
  local model
  for model in "${TEXT_MODELS[@]}"; do
    run_task 2 "$model/gaokao_2010_2022" \
      env CUDA_VISIBLE_DEVICES=2 "$VLLM_PYTHON" -m bench_coe.run_gaokao_text_smoke \
        --dataset gaokao_2010_2022 --model-path "models/$model" --model-name "$model" \
...
gpu3_chain() {
  local model
  for model in "${TEXT_MODELS[@]}"; do
    run_task 3 "$model/gaokao_2023_2024" \
      env CUDA_VISIBLE_DEVICES=3 "$VLLM_PYTHON" -m bench_coe.run_gaokao_text_smoke \
        --dataset gaokao_2023_2024 --model-path "models/$model" --model-name "$model" \
        --max-examples-per-task 0 --output-dir "$OUTPUT_ROOT/text/gaokao"
  done
...
log "front4 ready-model opportunistic scheduler started"
gpu0_chain & pid0=$!
gpu1_chain & pid1=$!
gpu2_chain & pid2=$!
gpu3_chain & pid3=$!
...
if [[ "$failed" -eq 0 ]]; then
  ...
  log "front4 ready-model opportunistic scheduler completed"
else
  log "front4 ready-model opportunistic scheduler finished with failures"
fi
exit "$failed"

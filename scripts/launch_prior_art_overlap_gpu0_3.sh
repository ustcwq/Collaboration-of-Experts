...
config="${1:-configs/innovation/prior_art_overlap_source_loso_gpu0_3.yaml}"
output_root="${2:-outputs/bench_coe/innovation/prior_art_overlap/source_loso_gpu0_3_v1_20260814}"
if [[ -e "$output_root" ]]; then
  echo "Refusing to overwrite prior-art overlap run root: $output_root" >&2
  exit 2
fi
...
seeds=(20260818 20260819 20260820 20260821)
pids=()
for gpu in 0 1 2 3; do
  seed="${seeds[$gpu]}"
  CUBLAS_WORKSPACE_CONFIG=:4096:8 PYTHONHASHSEED="$seed" CUDA_VISIBLE_DEVICES="$gpu" \
    python -m bench_coe.innovation.run_prior_art_overlap \
      --config "$config" \
      --seed "$seed" \
      --physical-gpu "$gpu" \
...
python -m bench_coe.innovation.aggregate_prior_art_overlap \
  --config "$config" \

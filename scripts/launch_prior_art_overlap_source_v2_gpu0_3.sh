...
config="${1:-configs/innovation/prior_art_overlap_source_loso_gpu0_3_v2.yaml}"
output_root="${2:-$(python -c 'import sys,yaml; print(yaml.safe_load(open(sys.argv[1]))["output_root"])' "$config")}"
if [[ -e "$output_root" ]]; then
  echo "Refusing to overwrite prior-art source run root: $output_root" >&2
  exit 2
fi
...
if (( ${#seeds[@]} != 4 )); then
  echo "Source config must declare exactly four seeds" >&2
  exit 4
fi

...
CONFIG=configs/innovation/c3_development_v5.yaml
RUN_ROOT=outputs/bench_coe/innovation/cross_examined_certificates/dev_v5_20260815
CERT_GENERATOR=Qwen2.5-7B-Instruct
CHECKER=General-Reasoner-7B-preview
SMOKE_WITNESSES=6
LOG_DIR="$ROOT/$RUN_ROOT/queue_logs"
CERT_LOG="$LOG_DIR/qwen_hardened_counterfactual_smoke_n${SMOKE_WITNESSES}.log"
CHECK_LOG="$LOG_DIR/general_hardened_parity_smoke_n${SMOKE_WITNESSES}.log"
...
while true; do
  ...
  {
    date '+C3 v5 hardened counterfactual smoke start %F %T'
  ...
  date '+C3 v5 hardened counterfactual startup/run failed; returning to idle queue %F %T' >> "$CERT_LOG"
done
...
jq -e \
  '(.parsed_witnesses / .witnesses) >= 0.90 and
   ...
   .prompt_version == "hardened_sealed_counterfactual_parity_v5" and
   .parser_version == "hardened_counterfactual_challenge_fields_v5"' \
...
date '+C3 v5 hardened counterfactual smoke finish %F %T' >> "$CERT_LOG"

smoke_check_witnesses=$(jq -r '.nonabstaining_witnesses' "$certificate_manifest")
while true; do
  gpu=$(wait_for_idle_gpu)
  {
    date '+C3 v5 hardened blind parity smoke start %F %T'
    echo "physical_gpu=$gpu"
  ...
  date '+C3 v5 hardened blind parity startup/run failed; returning to idle queue %F %T' >> "$CHECK_LOG"
done
...
jq -e \
  '(.parsed_reconstructions / .reconstructions) >= 0.90 and
   ...
   .counterfactual_pairs == true and
   .parity_orientations == ["canonical", "mirrored"] and
   .commitments_from_stage0 == true and
   .prompt_version == "hardened_blind_counterfactual_parity_v5" and
   .parser_version == "blind_counterfactual_parity_fields_v4"' \
...
jq -se \
  '(map(.dataset) | unique | length >= 2) and
   ...
   all(group_by(.witness_id)[];
     (map(.orientation) | unique | sort) == ["canonical", "mirrored"]) and
   all(group_by(.witness_id + "::" + .orientation)[];
     (map(.raw_output) | unique | length == 1) and
     (map(.prompt_sha256) | unique | length == 1))' \
  "$check_path" >/dev/null
date '+C3 v5 hardened blind parity smoke finish %F %T' >> "$CHECK_LOG"

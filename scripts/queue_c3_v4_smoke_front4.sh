...
CONFIG=configs/innovation/c3_development_v4.yaml
RUN_ROOT=outputs/bench_coe/innovation/cross_examined_certificates/dev_v4_20260815
CERT_GENERATOR=Qwen2.5-7B-Instruct
CHECKER=General-Reasoner-7B-preview
SMOKE_WITNESSES=6
LOG_DIR="$ROOT/$RUN_ROOT/queue_logs"
CERT_LOG="$LOG_DIR/qwen_counterfactual_parity_smoke_n${SMOKE_WITNESSES}.log"
CHECK_LOG="$LOG_DIR/general_blind_parity_smoke_n${SMOKE_WITNESSES}.log"
...
while true; do
  ...
  {
    date '+C3 v4 counterfactual-pair smoke start %F %T'
  ...
  date '+C3 v4 counterfactual-pair startup/run failed; returning to idle queue %F %T' >> "$CERT_LOG"
done
...
jq -e \
  '(.parsed_witnesses / .witnesses) >= 0.90 and
   ...
   .prompt_version == "sealed_counterfactual_parity_v4" and
   .parser_version == "sealed_counterfactual_challenge_fields_v4"' \
...
date '+C3 v4 counterfactual-pair smoke finish %F %T' >> "$CERT_LOG"

smoke_check_witnesses=$(jq -r '.nonabstaining_witnesses' "$certificate_manifest")
while true; do
  gpu=$(wait_for_idle_gpu)
  {
    date '+C3 v4 blind parity smoke start %F %T'
    echo "physical_gpu=$gpu"
  ...
  date '+C3 v4 blind parity startup/run failed; returning to idle queue %F %T' >> "$CHECK_LOG"
done
...
jq -e \
  '(.parsed_reconstructions / .reconstructions) >= 0.90 and
   ...
   .prompt_version == "blind_counterfactual_parity_v4" and
...
jq -se \
  ...
  "$check_path" >/dev/null
date '+C3 v4 blind parity smoke finish %F %T' >> "$CHECK_LOG"

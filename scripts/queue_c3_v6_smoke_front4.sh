...
CONFIG=configs/innovation/c3_development_v6.yaml
RUN_ROOT=outputs/bench_coe/innovation/cross_examined_certificates/dev_v6_20260815
...
CHECK_LOG="$LOG_DIR/general_commit_then_permute_parity_smoke_n${SMOKE_WITNESSES}.log"
...
while true; do
  ...
  {
    date '+C3 v6 commit-then-permute smoke start %F %T'
  ...
  date '+C3 v6 certificate startup/run failed; returning to idle queue %F %T' >> "$CERT_LOG"
done
...
date '+C3 v6 commit-then-permute smoke finish %F %T' >> "$CERT_LOG"

smoke_check_witnesses=$(jq -r '.nonabstaining_witnesses' "$certificate_manifest")
while true; do
  gpu=$(wait_for_idle_gpu)
  {
    date '+C3 v6 blind parity smoke start %F %T'
    echo "physical_gpu=$gpu"
  ...
  date '+C3 v6 blind parity startup/run failed; returning to idle queue %F %T' >> "$CHECK_LOG"
done
...
jq -se \
  ...
  "$check_path" >/dev/null
date '+C3 v6 blind parity smoke finish %F %T' >> "$CHECK_LOG"

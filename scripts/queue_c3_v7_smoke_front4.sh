...
CONFIG=configs/innovation/c3_development_v7.yaml
RUN_ROOT=outputs/bench_coe/innovation/cross_examined_certificates/dev_v7_20260815
...
CHECK_LOG="$LOG_DIR/general_isolated_trace_smoke_n${SMOKE_WITNESSES}.log"
...
while true; do
  ...
  {
    date '+C3 v7 commit-then-permute smoke start %F %T'
  ...
  date '+C3 v7 certificate startup/run failed; returning to idle queue %F %T' >> "$CERT_LOG"
done
...
date '+C3 v7 commit-then-permute smoke finish %F %T' >> "$CERT_LOG"

smoke_check_witnesses=$(jq -r '.nonabstaining_witnesses' "$certificate_manifest")
while true; do
  gpu=$(wait_for_idle_gpu)
  {
    date '+C3 v7 isolated trace smoke start %F %T'
    echo "physical_gpu=$gpu"
  ...
  date '+C3 v7 isolated trace startup/run failed; returning to idle queue %F %T' >> "$CHECK_LOG"
done
...
jq -e \
  '(.parsed_reconstructions / .reconstructions) >= 0.90 and
   ...
   .prompt_version == "blind_isolated_trace_audit_v7" and
...
jq -se \
  ...
  "$check_path" >/dev/null
date '+C3 v7 isolated trace smoke finish %F %T' >> "$CHECK_LOG"

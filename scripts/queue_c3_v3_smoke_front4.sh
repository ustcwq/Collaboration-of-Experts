#!/usr/bin/env bash
set -euo pipefail

ROOT=/home/sm5/ys/FCS
PYTHON_BIN=/home/sm5/anaconda3/envs/FactoryS/bin/python
CONFIG=configs/innovation/c3_development_v3.yaml
RUN_ROOT=outputs/bench_coe/innovation/cross_examined_certificates/dev_v3_20260815
CERT_GENERATOR=Qwen2.5-7B-Instruct
CHECKER=General-Reasoner-7B-preview
SMOKE_WITNESSES=6
LOG_DIR="$ROOT/$RUN_ROOT/queue_logs"
CERT_LOG="$LOG_DIR/qwen_sealed_effect_smoke_n${SMOKE_WITNESSES}.log"
CHECK_LOG="$LOG_DIR/general_blind_reconstruction_smoke_n${SMOKE_WITNESSES}.log"

mkdir -p "$LOG_DIR"
cd "$ROOT"

gpu_is_idle() {
  local gpu=$1
  local used
  local utilization
  IFS=, read -r used utilization < <(
    nvidia-smi -i "$gpu" \
      --query-gpu=memory.used,utilization.gpu \
      --format=csv,noheader,nounits
  )
  used=${used// /}
  utilization=${utilization// /}
  [[ "$used" -le 1024 && "$utilization" -le 10 ]]
}

wait_for_idle_gpu() {
  while true; do
    for gpu in 0 1 2 3; do
      if ! gpu_is_idle "$gpu"; then
        continue
      fi
      sleep 20
      if ! gpu_is_idle "$gpu"; then
        continue
      fi
      sleep 20
      if gpu_is_idle "$gpu"; then
        echo "$gpu"
        return 0
      fi
    done
    sleep 30
  done
}

while true; do
  gpu=$(wait_for_idle_gpu)
  {
    date '+C3 v3 sealed-effect smoke start %F %T'
    echo "physical_gpu=$gpu"
  } >> "$CERT_LOG"
  if env \
    CUDA_VISIBLE_DEVICES="$gpu" \
    PYTHONHASHSEED=20260815 \
    CUBLAS_WORKSPACE_CONFIG=:4096:8 \
    HF_HUB_OFFLINE=1 \
    TRANSFORMERS_OFFLINE=1 \
    "$PYTHON_BIN" -m bench_coe.innovation.run_c3_certificates \
      --config "$CONFIG" \
      --generator "$CERT_GENERATOR" \
      --physical-gpu "$gpu" \
      --smoke-questions "$SMOKE_WITNESSES" >> "$CERT_LOG" 2>&1; then
    break
  fi
  date '+C3 v3 sealed-effect startup/run failed; returning to idle queue %F %T' >> "$CERT_LOG"
done
certificate_dir="$RUN_ROOT/smoke/certificates/${CERT_GENERATOR}_n${SMOKE_WITNESSES}_gpu${gpu}"
certificate_path="$certificate_dir/certificates.jsonl"
certificate_manifest="$certificate_dir/certificate_manifest.json"
jq -e \
  '(.parsed_witnesses / .witnesses) >= 0.90 and
   .truncated_model_calls == 0 and
   .labels_read == false and
   .claims_are_sealed_from_checkers == true and
   .prompt_version == "sealed_effect_witness_v3" and
   .parser_version == "sealed_effect_set_fields_v3"' \
  "$certificate_manifest" >/dev/null
jq -se \
  '(map(.dataset) | unique | length >= 2) and
   (group_by(.witness_id) | length == 6) and
   all(group_by(.witness_id)[];
     (map(.raw_output) | unique | length == 1) and
     (map(.prompt_sha256) | unique | length == 1) and
     all(.[]; .claim_was_sealed == true))' \
  "$certificate_path" >/dev/null
date '+C3 v3 sealed-effect smoke finish %F %T' >> "$CERT_LOG"

while true; do
  gpu=$(wait_for_idle_gpu)
  {
    date '+C3 v3 blind reconstruction smoke start %F %T'
    echo "physical_gpu=$gpu"
  } >> "$CHECK_LOG"
  if env \
    CUDA_VISIBLE_DEVICES="$gpu" \
    PYTHONHASHSEED=20260815 \
    CUBLAS_WORKSPACE_CONFIG=:4096:8 \
    HF_HUB_OFFLINE=1 \
    TRANSFORMERS_OFFLINE=1 \
    "$PYTHON_BIN" -m bench_coe.innovation.run_c3_certificate_checks \
      --config "$CONFIG" \
      --checker "$CHECKER" \
      --physical-gpu "$gpu" \
      --certificate-path "$certificate_path" \
      --smoke-certificates "$SMOKE_WITNESSES" >> "$CHECK_LOG" 2>&1; then
    break
  fi
  date '+C3 v3 blind reconstruction startup/run failed; returning to idle queue %F %T' >> "$CHECK_LOG"
done
check_dir="$RUN_ROOT/smoke/checks/${CHECKER}_n${SMOKE_WITNESSES}_gpu${gpu}"
check_path="$check_dir/checks.jsonl"
check_manifest="$check_dir/check_manifest.json"
jq -e \
  '(.parsed_reconstructions / .reconstructions) >= 0.90 and
   .truncated_model_calls == 0 and
   .labels_read == false and
   .target_was_hidden == true and
   .sealed_claim_was_hidden == true and
   .commitments_from_stage0 == true and
   .prompt_version == "sealed_effect_reconstruction_v3" and
   .parser_version == "target_blind_effect_fields_v2"' \
  "$check_manifest" >/dev/null
jq -se \
  '(map(.dataset) | unique | length >= 2) and
   (group_by(.witness_id) | length == 6) and
   all(.[];
     .target_was_hidden == true and
     .sealed_claim_was_hidden == true and
     .commitment_source == "stage0_base_prediction") and
   all(group_by(.witness_id)[];
     (map(.raw_output) | unique | length == 1) and
     (map(.prompt_sha256) | unique | length == 1))' \
  "$check_path" >/dev/null
date '+C3 v3 blind reconstruction smoke finish %F %T' >> "$CHECK_LOG"
